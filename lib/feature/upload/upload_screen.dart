...
class _UploadScreenState extends State<UploadScreen> {
          ...
          actions: [
            TextButton(onPressed: () {
              if(path ==null && name.isEmpty){
                ShowErrorDialog(context, "Please upload image and enter your name",);
            ...
            }, child: Text("Done",style: gettitleTextStyle(fontsize: 18,color: AppColor.purpleColor),)),
  ...
  uploadImage({required bool isCamera}) async{
    await ImagePicker().pickImage(
        source:isCamera ? ImageSource.camera : ImageSource.gallery).then((value) {
      if(value!=null) {
        setState(() {
          path=value.path;
        });
